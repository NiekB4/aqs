...
Small modificications for working on LoPy made by Niek Bebelaar
...
# import pyb        # pyb is old. Use machine instead.
import machine
...
class Mpl115A2(object):
    ...
    def _send_command(self, command, value=None):
        ...
        if value is not None:
            for val in value:
                bvals.append(val)
        #self.bus.send(bvals, addr=self.address)  # send is old. Replace with writeto.
        #self.bus.readfrom(self.address, bvals)
        self.bus.writeto(self.address, bvals)
        self._last_command = command

    def _read_coefficients(self):
        self._send_command(0x04)
        # coefficients = self.bus.recv(8, addr=self.address) # recv is old. Replace with readfrom
        coefficients = self.bus.readfrom(self.address, 8)
    ...
    def _read_raw_pressure(self):
        ...
        # rp = self.bus.recv(2, addr=self.address) # recv is old. Replace with readfrom
        rp = self.bus.readfrom(self.address, 2)
        return int((rp[0] << 8 | rp[1]) >> 6)

    def _read_raw_temperature(self):
        self._send_command(0x02)
        # rt = self.bus.recv(2, addr=self.address) # recv is old. Replace with readfrom
        rt = self.bus.readfrom(self.address, 2)
